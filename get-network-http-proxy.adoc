...
a|Applies to both SVM and cluster-scoped objects. Either the UUID or name is supplied on input. This is mutually exclusive with SVM during POST and PATCH.
...
|svm
|link:#svm[svm]
a|This is mutually exclusive with IPspace during POST and PATCH.
