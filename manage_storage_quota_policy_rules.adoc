...
# The call:
curl -X GET "https://<mgmt-ip>/api/storage/quota/rules?rqtree.name=qt1&files.hard_limit=>5&#124;null" -H 'accept: application/hal+json'
...
"_links": {
  "self": {
    "href": "/api/storage/quota/rules?files.hard_limit=>5&#124;null"
  }
}
}
----
