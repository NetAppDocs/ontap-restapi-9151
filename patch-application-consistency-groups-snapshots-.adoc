...
summary: 'Complete a consistency group Snapshot copy operation'
---
...
|path
|True
a|Unique identifier of the consistency group's Snapshot copy for deletion.
...
|query
|False
a|Commits the Snapshot copy. The commit must be called within the timeout value specified during POST. If the commit is not called within the specified time, then the Snapshot copy create operation is aborted.
